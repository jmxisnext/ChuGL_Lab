...
OUT  = "01_PLANNING/zeta_sweep_metrics.csv"
...
def analyze_one(run_id):
    ...
    with open(path, newline="", encoding="utf-8") as f:
        ...
        for row in r:
            ...
            # For 2nd-order dynamics, control is the target position vector.
            err_pos_mag.append(mag3(px - d, py - s, pz - g))
            err_vel_mag.append(mag3(vx - d, vy - s, vz - g))

    if len(t) < 2:
        raise ValueError(f"{run_id}: insufficient rows")
    ...
    return {
        "Run_ID": run_id,
        "n_rows": len(t) + 1,  # +header line convention
        "t_start": t[0],
        "t_end": t[-1],
        "dt_mean": dt_mean,
...
def main():
    ...
    print(f"Wrote: {OUT}")
    for row in rows:
        print(f"{row['Run_ID']} ZETA={row['ZETA']}: err_pos_rms={row['err_pos_rms']:.6f} pos_rms={row['pos_rms']:.6f} vel_rms={row['vel_rms']:.6f} vel_energy={row['vel_energy']:.6f}")
